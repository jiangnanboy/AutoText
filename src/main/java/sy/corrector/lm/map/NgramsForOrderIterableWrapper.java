 ...
 * @param <W>
 */
public class NgramsForOrderIterableWrapper<W, V> implements Iterable<Entry<List<W>, V>>
{
	...
	public Iterator<Entry<List<W>, V>> iterator() {
		return new Iterators.Transform<NgramMap.Entry<V>, Entry<List<W>, V>>(map.getNgramsForOrder(ngramOrder).iterator())
		{

			@Override
			protected Entry<List<W>, V> transform(final NgramMap.Entry<V> next) {
				return new Entry<List<W>, V>()
				{
