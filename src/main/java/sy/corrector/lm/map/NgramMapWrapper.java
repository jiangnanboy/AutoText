...
public class NgramMapWrapper<W, V> extends AbstractMap<List<W>, V>
{
	...
	@Override
	public Set<Entry<List<W>, V>> entrySet() {
		return new AbstractSet<Entry<List<W>, V>>()
		{

			@Override
			public Iterator<Entry<List<W>, V>> iterator() {
				final Iterators.Transform<NgramsForOrderMapWrapper<W, V>, Iterator<Entry<List<W>, V>>> transform = new Iterators.Transform<NgramsForOrderMapWrapper<W, V>, Iterator<Entry<List<W>, V>>>(
					Arrays.asList(ngramsForOrder).iterator())
				{

					@Override
					protected Iterator<Entry<List<W>, V>> transform(final NgramsForOrderMapWrapper<W, V> next) {
