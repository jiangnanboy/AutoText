...
import sy.corrector.lm.collections.Iterators;
 ...
 * @param <W>
 */
public class NgramIterableWrapper<W, V> implements Iterable<Entry<List<W>, V>>
{
	...
	public Iterator<Entry<List<W>, V>> iterator() {
		final Iterators.Transform<NgramsForOrderIterableWrapper<W, V>, Iterator<Entry<List<W>, V>>> transform = new Iterators.Transform<NgramsForOrderIterableWrapper<W, V>, Iterator<Entry<List<W>, V>>>(
			Arrays.asList(ngramsForOrder).iterator())
		{

			@Override
			protected Iterator<Entry<List<W>, V>> transform(final NgramsForOrderIterableWrapper<W, V> next) {
				return next.iterator();
			}
		};
		return new Iterators.IteratorIterator<Entry<List<W>, V>>(transform);
